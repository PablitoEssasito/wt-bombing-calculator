...
/** One hardpoint preset from the armament data: the icon it states, and every chart bomb it hangs. */
 ...
 * the one a player actually sees — which also keeps a bomb's tile in a drop
 * schedule the same as on the creator's pylon. A preset's icon usually draws
 * the whole rack, so it is cut back to its single-round form, and counted only
 * when `known` says the game has an icon by that name. A tie goes to the icon
 * whose drag matches the bomb's `kinds` entry — the m/71 is drawn plain and
 * retarded equally often, and is a GP bomb.
 */
